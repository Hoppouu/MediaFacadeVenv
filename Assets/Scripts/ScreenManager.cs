...
public class ScreenManager : MonoBehaviour
{
    ...
    {
        ClearRenderTextureFunc(screens.main.renderTexture, Color.black);
        ClearRenderTextureFunc(screens.left.renderTexture, Color.black);
        ClearRenderTextureFunc(screens.right.renderTexture, Color.black);
        ClearRenderTextureFunc(screens.floor.renderTexture, Color.black);
    }
    private void ClearRenderTextureFunc(RenderTexture rt, Color clearColor)
    {
    ...
    private void InitScreen(ref VideoScreen vs)
    {
        ...
        vs.renderTexture = vs.meshRenderer.material.mainTexture as RenderTexture;
        vs.boxCollider = vs.screen.GetComponent<BoxCollider>();
        vs.ndiReceiver = vs.screen.GetComponent<NdiReceiver>();
    ...
    private void SetSettingFunc(ref VideoScreen videoScreen, ScreenSetting setting)
    {
        ...
        videoScreen.meshRenderer.enabled = setting.stateScreen;
        videoScreen.boxCollider.enabled = setting.stateScreen;
        videoScreen.ndiReceiver.enabled = setting.stateNDI;
        videoScreen.ndiReceiver.ndiName = setting.ndiName;

        if (setting.stateScreen)    videoScreen.videoPlayer.audioOutputMode = VideoAudioOutputMode.AudioSource;
    ...
    public struct VideoScreen
    {
        ...
        [HideInInspector] public NdiReceiver ndiReceiver;
    };
